...
import { AuthProvider, useAuth } from "@/hooks/identity/use-auth";
...
import {
  ...
  loginWithGoogle,
  refreshAccessToken,
} from "@/lib/services/identity/auth";
...
function AuthProbe() {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => void googleLogin("google-token")}>
        Google login
      </button>
    </div>
  );
...
describe("stored-session recovery", () => {
  ...
  it("applies a Google-authenticated session through the same access boundary", async () => {
    vi.mocked(getCurrentAccount).mockResolvedValueOnce(currentAccount);
    vi.mocked(loginWithGoogle).mockResolvedValueOnce(refreshedSession);
    ...
    await waitFor(() => expect(readAuthSession()?.accessToken).toBe("refreshed-access-token"));
