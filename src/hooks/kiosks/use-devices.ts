...
export function useDevices(kioskId: string) {
  ...
  const replaceDevice = useCallback(
    async (deviceId: string, replacementDeviceId: string, reason: string) => {
      if (mutationInFlightRef.current) return null;
      mutationInFlightRef.current = true;
      setIsMutating(true);
      setMutationErrorMessage(null);
      try {
        const result = await replaceManagementDevice(kioskId, deviceId, {
          replacementDeviceId,
          reason,
        });
        await fetchDevices();
        toast.success(
          `Đã thay thiết bị và chuyển ${result.reboundContainerCount} cấu hình dispenser.`,
        );
        return result;
      } catch (error) {
        const message = getDeviceManagementErrorMessage(error);
