...
export function useConfigurationReleases(organizationId: string) {
  ...
  const refresh = useCallback(async (
    ...
    try {
      ...
      if (!signal?.aborted && reportAsPrimaryError) {
        setErrorMessage(getProductionOperationsErrorMessage(error, "Không thể tải bản phát hành cấu hình."));
      }
      return false;
    } finally {
  ...
  const runMutation = useCallback(async <T,>(mutation: () => Promise<T>, success: string) => {
    ...
    try {
      ...
      if (!refreshed) {
        setErrorMessage(null);
        const warning = "Thao tác đã thành công nhưng danh sách chưa tải lại được. Hãy thử làm mới.";
  ...
  const loadEditor = useCallback(async (releaseId: string) => {
    ...
    try {
      ...
      if (requestId === detailRequestRef.current) {
        setErrorMessage(getProductionOperationsErrorMessage(error, "Không thể mở trình soạn bản phát hành."));
      }
      return null;
    }
  ...
  return {
    ...
    refresh,
    loadEditor,
    cancelEditorLoad: () => { detailRequestRef.current += 1; },
