...
describe("device management contracts", () => {
  ...
  it("uses the kiosk-owned device replacement contract", async () => {
    vi.mocked(axiosClient.post).mockResolvedValue({
      data: {
        ...
        data: {
          ...
          replacementDeviceId: "device-2",
          reboundContainerCount: 2,
          replacementDispenserStateIds: ["state-1", "state-2"],
        },
      },
    } as AxiosResponse);

    await replaceManagementDevice("kiosk-1", "device-1", {
      replacementDeviceId: "device-2",
      reason: "Thay thiết bị hỏng",
    });

    expect(axiosClient.post).toHaveBeenCalledWith(
      "/api/v1/management/kiosks/kiosk-1/devices/device-1/replace",
      { replacementDeviceId: "device-2", reason: "Thay thiết bị hỏng" },
    );
  });
});
