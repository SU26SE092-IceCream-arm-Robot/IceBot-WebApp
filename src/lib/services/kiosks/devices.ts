...
export async function replaceManagementDevice(
  kioskId: string,
  deviceId: string,
  request: ReplaceDeviceRequest,
): Promise<DeviceReplacementResult> {
  const response = await axiosClient.post<ApiResult<DeviceReplacementResult>>(
    `/api/v1/management/kiosks/${encodeURIComponent(kioskId)}/devices/${encodeURIComponent(deviceId)}/replace`,
    request,
  );

  return requireData(response.data, "Không thể thay thế thiết bị.");
}
