...
import type {
  ...
  ManualPaymentSessionReconciliationResult,
  OpenProductionIncidentRequest,
  PaymentSessionInterventionResult,
  PaymentSessionInterventionsQuery,
...
export async function listPaymentSessionInterventions(
  query: PaymentSessionInterventionsQuery,
  signal?: AbortSignal,
): Promise<TransactionsPagedResult<PaymentSessionInterventionResult>> {
  const response = await axiosClient.get<
    TransactionsPagedResult<PaymentSessionInterventionResult>
  ...
  return requirePagedData(
    response.data,
    "Không thể tải danh sách thanh toán cần can thiệp.",
  );
}
...
export async function reconcilePaymentSession(
  ...
  reason: string,
): Promise<ManualPaymentSessionReconciliationResult> {
  const response = await axiosClient.post<
    ApiResult<ManualPaymentSessionReconciliationResult>
  >(
    `/api/v1/management/orders/${encodeURIComponent(orderId)}/payment-transactions/${encodeURIComponent(paymentTransactionId)}/reconcile`,
    { reason },
  );

  return requireData(response.data, "Không thể đối soát lại phiên thanh toán.");
}

export async function redispatchOrderExecution(
  orderId: string,
  reason: string,
): Promise<unknown> {
  ...
  if (!response.data.succeeded) {
    throw new Error(response.data.message || "Không thể gửi lại yêu cầu sản xuất.");
  }
  return response.data.data;
}
...
export async function openProductionIncident(
  orderId: string,
  orderItemId: string,
  request: OpenProductionIncidentRequest,
): Promise<ProductionIncidentResult> {
  const response = await axiosClient.post<ApiResult<ProductionIncidentResult>>(
    `/api/v1/management/orders/${encodeURIComponent(orderId)}/items/${encodeURIComponent(orderItemId)}/production-incidents`,
    request,
  );

  return requireData(response.data, "Không thể ghi nhận sự cố sản xuất.");
}
