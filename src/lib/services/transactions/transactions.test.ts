...
describe("management transaction GraphQL contracts", () => {
  ...
  it("uses the payment intervention discovery and manual reconciliation contracts", async () => {
    vi.mocked(axiosClient.get).mockResolvedValue({
      data: { succeeded: true, statusCode: 200, data: [], pagination: pageInfo },
    } as AxiosResponse);
    vi.mocked(axiosClient.post).mockResolvedValue({
      data: {
        succeeded: true,
        statusCode: 200,
        data: { paymentTransactionId: "payment-1", orderId: "order-1", outcome: "Reconciled" },
      },
    } as AxiosResponse);

    await listPaymentSessionInterventions({
      interventionCode: "ProviderStatusUnknown",
      pageNumber: 1,
      pageSize: 20,
    });
    await reconcilePaymentSession("order-1", "payment-1", "Kiểm tra trạng thái với PayOS");

    expect(axiosClient.get).toHaveBeenCalledWith(
      "/api/v1/management/payment-session-interventions",
      expect.objectContaining({
        params: expect.objectContaining({ interventionCode: "ProviderStatusUnknown" }),
      }),
    );
    expect(axiosClient.post).toHaveBeenCalledWith(
      "/api/v1/management/orders/order-1/payment-transactions/payment-1/reconcile",
      { reason: "Kiểm tra trạng thái với PayOS" },
    );
  });

  it("uses evidence-owned order recovery contracts", async () => {
    vi.mocked(axiosClient.post).mockResolvedValue({
      data: { succeeded: true, statusCode: 200, data: { id: "incident-1" } },
    } as AxiosResponse);

    await redispatchOrderExecution("order-1", "Edge đã sẵn sàng trở lại");
    await openProductionIncident("order-1", "item-1", {
      sourceCommandId: "command-1",
      sourceProductionJobId: "job-1",
      productionUnitNo: 2,
      productionUnitQuantity: 1,
      reason: "Ly số 2 bị lỗi",
    });

    expect(axiosClient.post).toHaveBeenNthCalledWith(
      1,
      "/api/v1/management/orders/order-1/execution-attempts",
      { reason: "Edge đã sẵn sàng trở lại" },
    );
    expect(axiosClient.post).toHaveBeenNthCalledWith(
      2,
      "/api/v1/management/orders/order-1/items/item-1/production-incidents",
      expect.objectContaining({ sourceCommandId: "command-1", sourceProductionJobId: "job-1" }),
    );
  });
});
