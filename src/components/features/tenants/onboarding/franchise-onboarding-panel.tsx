...
export function FranchiseOnboardingPanel({
  ...
  const start = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!isValidIanaTimeZone(timeZone)) {
      setValidationMessage("Múi giờ không hợp lệ. Hãy dùng định dạng IANA, ví dụ Asia/Ho_Chi_Minh.");
      return;
    }
    ...
    <Card className="gap-0 rounded-xl border border-border/80 py-0 shadow-none">
      <CardHeader className="border-b border-border px-5 py-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><CardTitle className="text-base font-semibold">Thiết lập nhanh điểm bán</CardTitle><p className="mt-1 text-xs text-muted-foreground">Tạo đồng thời một cửa hàng và một kiosk mới trong quy trình có thể tiếp tục khi gián đoạn. Dữ liệu được tạo trực tiếp vẫn được quản lý bình thường và không xuất hiện trong lịch sử này.</p></div>
          <div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => void workflow.refresh()} disabled={workflow.isLoading || workflow.isMutating}><RefreshCw className={workflow.isLoading ? "size-4 animate-spin" : "size-4"} />Làm mới</Button>{canStart ? <Button size="sm" onClick={() => { workflow.clearError(); setStartOpen(true); }} disabled={workflow.isMutating}><Plus className="size-4" />Tạo nhanh</Button> : null}</div>
        </div>
      </CardHeader>
      <CardContent className="space-y-3 p-5">
        ...
        {!workflow.isLoading && workflow.items.length === 0 ? <p className="text-sm text-muted-foreground">Chưa có lịch sử thiết lập nhanh. Cửa hàng hoặc kiosk được tạo trực tiếp không xuất hiện tại đây.</p> : null}
