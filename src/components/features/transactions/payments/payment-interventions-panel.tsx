...
export function PaymentInterventionsPanel({ enabled }: { enabled: boolean }) {
  ...
  const [target, setTarget] = useState<PaymentSessionInterventionResult | null>(
    null,
  );
    ...
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        setErrorMessage(
          getTransactionsErrorMessage(
            error,
            "Không thể tải thanh toán cần can thiệp.",
          ),
        );
      } finally {
  ...
  const reconcile = async () => {
    ...
    try {
      ...
      setValidationMessage(
        getTransactionsErrorMessage(
          error,
          "Không thể đối soát lại thanh toán.",
        ),
      );
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Các phiên thanh toán mà backend không thể tự kết luận và cần nhân
            viên đối soát với nhà cung cấp.
          </p>
        </div>
        <Button
          variant="outline"
            ...
            >
              <div className="min-w-0 space-y-1">
                <p className="font-medium">
                  {item.orderNumber} · {item.provider}
                </p>
                ...
                <p className="text-xs text-muted-foreground">
                  {formatTransactionMoney(item.amount, item.currency)} · Yêu cầu{" "}
                  {formatTransactionDate(item.requestedAt)} · Thử{" "}
      ...
      <Dialog
        open={target !== null}
        onOpenChange={(open) => {
          if (!open && !isReconciling) setTarget(null);
        }}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Backend sẽ hỏi lại nhà cung cấp và lưu người thao tác cùng lý do.
              Thao tác không tự đánh dấu đã thanh toán nếu provider chưa xác
              nhận.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button isLoading={isReconciling} onClick={() => void reconcile()}>
              Xác nhận đối soát
            </Button>
          </DialogFooter>
        </DialogContent>
