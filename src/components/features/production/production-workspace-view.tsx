  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-col gap-4 border-b border-border pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex items-start gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-lg border border-primary/20 bg-primary/10 text-primary"><Factory className="size-5" /></span><div><h1 className="text-3xl font-semibold tracking-tight">Cấu hình sản xuất</h1><p className="mt-1 text-sm text-muted-foreground">Nhập chương trình Fairino, kiểm tra nội dung và liên kết với cấu hình sản phẩm.</p></div></div>
        <Button variant="outline" disabled={scope.isLoading} onClick={() => void scope.refresh()}><RefreshCw className="size-4" />Làm mới</Button>
      </section>

      {!selected ? <div className="space-y-4"><div className="flex justify-end"><ProductionWorkflowStepper currentStep={1} completedSteps={[]} organizationName={null} /></div><Card className="border-border/80 shadow-none"><CardHeader><CardTitle className="text-base">Bước 1: Chọn tổ chức</CardTitle><CardDescription>Chọn tổ chức để xem các bundle mà backend cho phép tài khoản hiện tại truy cập.</CardDescription></CardHeader><CardContent className="space-y-3"><Label htmlFor="production-organization">Tổ chức</Label><Select value={scope.selectedOrganizationId ?? ""} onValueChange={(value) => scope.setSelectedOrganizationId(value ?? null)} disabled={scope.isLoading || scope.organizations.length === 0}><SelectTrigger id="production-organization" className="w-full sm:max-w-xl"><SelectValue>{scope.isLoading ? "Đang tải tổ chức..." : "Chọn tổ chức"}</SelectValue></SelectTrigger><SelectContent>{scope.organizations.map((organization) => <SelectItem key={organization.id} value={organization.id}>{organizationLabel(organization.name, organization.code)}</SelectItem>)}</SelectContent></Select>{scope.errorMessage ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">{scope.errorMessage}</div> : null}{!scope.isLoading && !scope.errorMessage && scope.organizations.length === 0 ? <p className="text-sm text-muted-foreground">Tài khoản hiện tại chưa có tổ chức nào trong phạm vi cấu hình sản xuất.</p> : null}</CardContent></Card></div> : null}

      {selected && canRead ? <div className="space-y-5"><Card className="border-border/80 shadow-none"><CardHeader><CardTitle className="text-base">Tổ chức đang cấu hình</CardTitle><CardDescription>Đổi tổ chức sẽ tải lại danh sách và workspace theo phạm vi mới.</CardDescription></CardHeader><CardContent><Select value={scope.selectedOrganizationId ?? ""} onValueChange={(value) => scope.setSelectedOrganizationId(value ?? null)} disabled={scope.isLoading}><SelectTrigger id="production-organization" className="w-full sm:max-w-xl"><SelectValue>{organizationLabel(selected.name, selected.code)}</SelectValue></SelectTrigger><SelectContent>{scope.organizations.map((organization) => <SelectItem key={organization.id} value={organization.id}>{organizationLabel(organization.name, organization.code)}</SelectItem>)}</SelectContent></Select></CardContent></Card><RobotAuthoringImportsPanel key={selected.id} organizationId={selected.id} organizationName={organizationLabel(selected.name, selected.code)} canRead={canRead} canUpload={canUpload} canManagePrograms={canManagePrograms} canManageReleases={canManageReleases} /></div> : null}
      {selected && !canRead ? <div className="space-y-4"><div className="flex justify-end"><ProductionWorkflowStepper currentStep={1} completedSteps={[1]} organizationName={organizationLabel(selected.name, selected.code)} /></div><div className="rounded-lg border border-dashed px-5 py-10 text-center text-sm text-muted-foreground">Bạn không có quyền xem cấu hình sản xuất của tổ chức đã chọn.</div></div> : null}
    </div>
  );
}
