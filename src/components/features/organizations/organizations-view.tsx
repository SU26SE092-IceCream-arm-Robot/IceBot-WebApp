  ...
  return (
    <div className="space-y-7">
      ...
      <Card className="gap-0 border border-border/80 py-0 shadow-none">
        <CardContent className="grid gap-2 bg-muted/10 p-4 md:grid-cols-[minmax(260px,1fr)_220px_auto]">
          <form className="relative" onSubmit={(event) => { event.preventDefault(); setPage(1); setSearch(searchDraft.trim()); }}><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={searchDraft} placeholder="Tìm theo tên hoặc mã tổ chức..." className="h-9 bg-card pl-9" onChange={(event) => setSearchDraft(event.target.value)} /></form>
          <Select value={status} onValueChange={(value) => { if (STATUS_OPTIONS.some((option) => option.value === value)) { setStatus(value as TenantStatusFilter); setPage(1); } }}><SelectTrigger className="h-9 w-full bg-card"><SelectValue>{STATUS_OPTIONS.find((option) => option.value === status)?.label}</SelectValue></SelectTrigger><SelectContent>{STATUS_OPTIONS.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select>
          <Button variant="outline" size="sm" onClick={() => { setSearchDraft(""); setSearch(""); setStatus("ALL"); setPage(1); }}>Xóa lọc</Button>
        </CardContent>
      </Card>

      <Card className="gap-0 border border-border/80 py-0 shadow-none">
        <CardHeader className="border-b border-border py-4"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl border border-primary/20 bg-primary/10 text-primary"><Building2 className="size-5" /></span><CardTitle>Danh sách tổ chức</CardTitle></div></CardHeader>
        {isLoading ? <TenantLoadingState label="Đang tải tổ chức..." /> : errorMessage ? <TenantErrorState message={errorMessage} onRetry={() => void fetchOrganizations()} /> : organizations.length === 0 ? <TenantEmptyState title="Chưa có tổ chức phù hợp" description="Thử thay đổi bộ lọc hoặc tạo tổ chức mới." /> : (
          <Table className="min-w-[900px] table-fixed"><TableHeader><TableRow><TableHead className="w-[28%] px-4">Tổ chức</TableHead><TableHead className="w-[24%]">Liên hệ</TableHead><TableHead className="w-[16%] text-center">Trạng thái</TableHead><TableHead className="w-[18%] text-center">Cập nhật</TableHead><TableHead className="w-[14%] px-4 text-center">Thao tác</TableHead></TableRow></TableHeader><TableBody>{organizations.map((organization) => <TableRow key={organization.id}><TableCell className="px-4 py-3"><p className="font-medium">{organization.name}</p><p className="font-mono text-xs text-muted-foreground">{organization.code}</p></TableCell><TableCell><p className="truncate">{organization.email || "Chưa có email"}</p><p className="text-xs text-muted-foreground">{organization.phoneNumber || "Chưa có số điện thoại"}</p></TableCell><TableCell className="text-center"><div className="flex justify-center"><TenantStatusBadge status={organization.status} /></div></TableCell><TableCell className="text-center text-xs text-muted-foreground">{formatTenantDate(organization.updatedAt ?? organization.createdAt)}</TableCell><TableCell className="px-4"><div className="flex justify-center gap-1"><Link href={`/organizations/${organization.id}`} className={cn(buttonVariants({ variant: "ghost", size: "icon-sm" }), "text-muted-foreground")} title="Xem chi tiết"><Eye className="size-4" /></Link>{canEdit ? <Button variant="ghost" size="icon-sm" title="Chỉnh sửa" onClick={() => { setEditingOrganization(organization); setMutationError(null); setFormOpen(true); }}><Pencil className="size-4" /></Button> : null}{isSystemAdmin ? <Button variant="ghost" size="icon-sm" className={organization.status === "Active" ? "text-destructive" : "text-success"} title={organization.status === "Active" ? "Vô hiệu hóa" : "Kích hoạt"} onClick={() => { setMutationError(null); setLifecycleTarget({ organization, activate: organization.status !== "Active" }); }}>{organization.status === "Active" ? <PowerOff className="size-4" /> : <Power className="size-4" />}</Button> : null}</div></TableCell></TableRow>)}</TableBody></Table>
        )}
        <div className="flex items-center justify-between border-t border-border px-4 py-3 text-sm"><p className="text-muted-foreground">Trang <span className="font-medium text-foreground">{page}</span> / {Math.max(totalPages, 1)} · {totalCount} tổ chức</p><div className="flex gap-2"><Button variant="outline" size="sm" disabled={page <= 1 || isLoading} onClick={() => setPage((value) => Math.max(1, value - 1))}><ChevronLeft className="size-4" />Trước</Button><Button variant="outline" size="sm" disabled={page >= totalPages || isLoading} onClick={() => setPage((value) => value + 1)}>Sau<ChevronRight className="size-4" /></Button></div></div>
      </Card>
