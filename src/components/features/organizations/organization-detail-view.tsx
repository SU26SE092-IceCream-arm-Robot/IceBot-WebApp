...
export function OrganizationDetailView({ organizationId }: OrganizationDetailViewProps) {
  ...
  return (
    <div className="space-y-7">
      {successMessage ? <div role="status" className="flex items-center justify-between gap-3 rounded-lg border border-success/30 bg-success/5 px-4 py-3 text-sm text-success"><span>{successMessage}</span><Button variant="ghost" size="sm" className="h-7 text-success" onClick={() => setSuccessMessage(null)}>Đóng</Button></div> : null}
      ...
      <Card className="border border-border/80 shadow-none"><CardHeader className="border-b border-border"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl border border-primary/20 bg-primary/10 text-primary"><Building2 className="size-5" /></span><CardTitle>Thông tin tổ chức</CardTitle></div></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><DetailField label="Tên pháp lý" value={organization.legalName || "Chưa có"} /><DetailField label="Mã số thuế" value={organization.taxCode || "Chưa có"} /><DetailField label="Email" value={organization.email || "Chưa có"} /><DetailField label="Số điện thoại" value={organization.phoneNumber || "Chưa có"} /><DetailField label="Địa chỉ" value={organization.address || "Chưa có"} /><DetailField label="Cập nhật" value={formatTenantDate(organization.updatedAt ?? organization.createdAt)} /></CardContent></Card>

      <Card className="gap-0 border border-border/80 py-0 shadow-none"><CardHeader className="border-b border-border py-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl border border-primary/20 bg-primary/10 text-primary"><StoreIcon className="size-5" /></span><div><CardTitle>Cửa hàng trực thuộc</CardTitle><p className="text-sm text-muted-foreground">{stores.length} cửa hàng</p></div></div>{canManageStores && organization.status === "Active" ? <Button onClick={() => { setMutationError(null); setStoreFormOpen(true); }}><Plus className="size-4" />Tạo cửa hàng</Button> : null}</div></CardHeader>
        {stores.length === 0 ? <TenantEmptyState title="Chưa có cửa hàng" description="Tổ chức này chưa có cửa hàng trực thuộc." /> : <Table className="min-w-[850px] table-fixed"><TableHeader><TableRow><TableHead className="w-[28%] px-4">Cửa hàng</TableHead><TableHead className="w-[24%]">Địa điểm</TableHead><TableHead className="w-[18%] text-center">Trạng thái</TableHead><TableHead className="w-[16%] text-center">Múi giờ</TableHead><TableHead className="w-[14%] px-4 text-center">Thao tác</TableHead></TableRow></TableHeader><TableBody>{stores.map((store) => <TableRow key={store.id}><TableCell className="px-4 py-3"><p className="font-medium">{store.name}</p><p className="font-mono text-xs text-muted-foreground">{store.code}</p></TableCell><TableCell><p className="truncate">{store.address || "Chưa có địa chỉ"}</p><p className="text-xs text-muted-foreground">{[store.city, store.province].filter(Boolean).join(", ") || "Chưa có địa phương"}</p></TableCell><TableCell><div className="flex justify-center"><TenantStatusBadge status={store.status} /></div></TableCell><TableCell className="text-center font-mono text-xs">{store.timeZone}</TableCell><TableCell><div className="flex justify-center gap-1"><Link href={`/stores/${store.id}`} className={cn(buttonVariants({ variant: "ghost", size: "icon-sm" }), "text-muted-foreground")} title="Xem cửa hàng"><Eye className="size-4" /></Link>{canManageStores ? <Button variant="ghost" size="icon-sm" className={store.status === "Active" ? "text-destructive" : "text-success"} title={store.status === "Active" ? "Vô hiệu hóa" : "Kích hoạt"} onClick={() => { setMutationError(null); setLifecycleTarget({ kind: "store", store, activate: store.status !== "Active" }); }}>{store.status === "Active" ? <PowerOff className="size-4" /> : <Power className="size-4" />}</Button> : null}</div></TableCell></TableRow>)}</TableBody></Table>}
      </Card>
